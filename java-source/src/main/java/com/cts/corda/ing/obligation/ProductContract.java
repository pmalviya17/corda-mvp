...
public class ProductContract implements Contract {
    ...
    public static final String OBLIGATION_CONTRACT_ID = "com.cts.corda.ing.obligation.ProductContract";

    public static final String PRODUCT_BUY_CONTRACT = "com.cts.corda.ing.contract.BuyContract";

    public static final String PRODUCT_SELL_CONTRACT = "com.cts.corda.ing.contract.SellContract";

    public static final String PRODUCT_CASH_SWAP_CONTRACT = "com.cts.corda.ing.contract.EtfSwapContract"; //This is for DTC
    ...
    public void verify(LedgerTransaction tx) {
        ...
        if (commandData instanceof Commands.Issue) {
            verifyProductIssue(tx, setOfSigners);
    ...
    private Set<PublicKey> keysFromParticipants(ProductObligation obligation) {
        return obligation
    ...
    private void verifyProductIssue(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            ProductObligation obligation = (ProductObligation) tx.getOutputStates().get(0);

            //req.using("A newly issued obligation must have a positive amount.", obligation.getAmount().getQuantity() > 0);
            req.using("A newly issued PRODUCT obligation must have a positive quantity.", obligation.getProductAsset().getQuantity() > 0);
    ...
    private void verifyTransfer(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            ProductObligation input = tx.inputsOfType(ProductObligation.class).get(0);
            ProductObligation output = tx.outputsOfType(ProductObligation.class).get(0);
    ...
    private void verifySettle(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            List<ProductObligation> obligationInputs = tx.inputsOfType(ProductObligation.class);
            ...
            ProductObligation inputObligation = obligationInputs.get(0);
