...
angular.module('demoAppModule', ['ui.bootstrap']).controller('DemoAppCtrl', function($http, $location, $uibModal) {
    ...
    /** Displays the Product creation modal. */
    demoApp.openBuySellProductModal = () => {
        const createProductModal = $uibModal.open({
            templateUrl: 'createProductModal.html',
            controller: 'CreateProductModalCtrl',
            controllerAs: 'createProductModal',
            resolve: {
                apiBaseURL: () => apiBaseURL,
                peers: () => peers
            }
        ...
        // Ignores the modal result events.
        createProductModal.result.then(() => {}, () => {});
    };
    ...
    /** Displays the cash issuance modal. */
    demoApp.openSelfIssueProductModal = () => {
        const selfIssueProductModal = $uibModal.open({
            templateUrl: 'selfIssueProductModal.html',
            controller: 'SelfIssueProductModalCtrl',
            controllerAs: 'selfIssueProductModal',
            resolve: {
                apiBaseURL: () => apiBaseURL
            }
        });

        selfIssueProductModal.result.then(() => {}, () => {});
    };
    /** Displays the cash issuance modal. */
    ...
    demoApp.refresh = () => {
        ...
        $http.get(apiBaseURL + "checkProductBalance").then((response) => demoApp.checkProductBalance =
            response.data);
